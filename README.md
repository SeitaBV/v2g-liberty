# V2G Liberty: A HomeAssistant-based app to optimize vehicle-to-grid charging of your EV
...
The schedules are optimized on EPEX day-ahead energy prices, so this works best with an electricity contract with dynamic (hourly) prices (for now: only Dutch energy prices, if you have interest in this software and live outside the Netherlands, please [contact us](https://v2g-liberty.eu/)).
...
- In automatic mode, FlexMeasures is periodically asked to generate optimized charging schedules. V2G Liberty translates this into set points which it sends to the Wallbox Quasar via modbus.
...
So before you can start with V2G Liberty you'll need a running HA in your network.
...
Preferably name the calendar car_reservation. Otherwise, update the calendar name in the configuration of V2G Liberty.
...
You'll need to register these modules in HA.
Go to Settings > Dashboards > Resources (top right).
Add resource as Javascript -module with URL’s:
...
## Configure HA with (secret) settings
This is not complicated, but you'll need to work precise.
In the HA file editor open `/config/secrets.yaml` and the following code, replacing secrets/values as required for your custom setting.
...
# If a load-balancer (Power Boost for WB) is installed it is safe to use max amperage
# of the phase * 233 (Volt). So for 25 A, this is 5825 W.
# If there is no load-balancer, please ask your installer what max power can be
# used by the wallbox. It's very rare that discharge power differs from charge power.
...
## Copy & edit files
Configuration of HA is done with .yaml files.
These can be edited in the file editor in HA.
Or, if you have installed the Studio Code Server addon (not mandatory!), you can use that.

These steps will take focus, but nothing that is complicated.
...
<<TODO: add picture of folder structure with files...>>
...
In your Home Assistant file editor, go to `/config/configuration.yaml` add the following to the top of the file:
